...
__attribute__((interrupt)) void keyboard_handler(struct interrupt_frame* frame)
{
	...
	// basically just put the contents from inside the while loop from hw2 into here.

	// just need this so it can run
	(void)frame;
	// read the scancode from th ps2 data port which is 0x60
	uint8_t sc = inb(0x60);
	if (sc & KEY_RELEASED)
		key_state[sc & ~KEY_RELEASED] = 0;
	else
		key_state[sc] = 1;
	outb(0x20, 0x20);
}
